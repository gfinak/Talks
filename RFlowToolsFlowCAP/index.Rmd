...
```{r eval=c(1,2,4,5),echo=FALSE,cache=TRUE,result='hide',message=FALSE,warning=FALSE,error=FALSE}
...
```
```{r echo=FALSE,eval=TRUE,cache=TRUE,error=FALSE,message=FALSE,warning=FALSE,results='hide'}
...
wsfile<-list.files("./Data/",pattern="xml",full=TRUE)
```

```{r eval=TRUE,echo=FALSE,message=FALSE,results='hide',warning=FALSE,error=FALSE}
ws<-openWorkspace(wsfile)
G<-parseWorkspace(wsfile)
if(!file.exists("Data/gs1")){
G<-lapply(2:10,function(i)parseWorkspace(ws,name=i))
sapply(1:length(G),function(i)save_gs(G[[i]],path=sprintf("./Data/gs%s",i)))
}else{
  G<-lapply(1:9,function(i)load_gs(sprintf("Data/gs%s",i)))
}
```
```{r plotws,eval=TRUE,echo=FALSE,cache=TRUE,results='hide',fig.show=FALSE,fig.keep='none',fig.width=7,fig.height=6}
plot(G[[1]][[1]],2)
plotGate(G[[1]][[1]],4,xbin=128,lwd=2)
```
